...
class OllamaClient:
    ...
    def answer(
        ...
        try:
            logger.debug(f"Gửi request đến Ollama: {self.base_url}/api/generate, model: {self.model_name}")
            ...
            logger.debug(f"Ollama done_reason: {result.get('done_reason', 'N/A')}")
            logger.debug(f"Ollama done: {result.get('done', 'N/A')}")
            ...
            if not answer or not answer.strip():
                logger.warning(f"Ollama trả về response rỗng. done_reason: {done_reason}, done: {done}, result keys: {list(result.keys())}")
                ...
                if done_reason == "stop" and not answer:
                    ...
                    return "Câu trả lời bị cắt ngắn do giới hạn độ dài. Vui lòng thử lại với câu hỏi ngắn hơn.", {"model": self.model_name, "warning": "truncated"}
                elif not done:
                    return "Ollama đang xử lý nhưng chưa hoàn thành. Vui lòng thử lại sau.", {"model": self.model_name, "warning": "not_done"}
                else:
                    return "Không nhận được phản hồi từ Ollama. Vui lòng thử lại.", {"model": self.model_name, "warning": "empty_response"}
            
            logger.debug(f"Ollama trả về response thành công, độ dài: {len(answer)}")
            return answer, {"model": self.model_name}
        except requests.exceptions.Timeout as e:
            logger.error(f"Ollama timeout: {str(e)}")
            return f"Ollama không phản hồi trong thời gian cho phép (timeout). Vui lòng thử lại.", {"model": self.model_name, "error": "timeout"}
        except requests.exceptions.ConnectionError as e:
            logger.error(f"Ollama connection error: {str(e)}")
            return f"Không thể kết nối đến Ollama server tại {self.base_url}. Vui lòng kiểm tra kết nối mạng hoặc liên hệ quản trị viên.", {"model": self.model_name, "error": "connection_error"}
    ...
    def analyze_image_with_gemma3(
        ...
        """
        Phân tích ảnh bằng Gemma3 model qua Ollama.
        ...
        # Chỉ lấy ảnh đầu tiên (Ollama có thể hỗ trợ nhiều ảnh nhưng để đơn giản dùng 1 ảnh)
        ...
        # Ollama /api/chat format cho vision
        payload = {
        ...
        try:
            logger.debug(f"Gửi request đến Ollama vision: {self.base_url}/api/chat, model: {self.vision_model_name}")
            response = requests.post(
            ...
            logger.error(f"Ollama vision timeout: {str(e)}")
            return "", {"model": self.vision_model_name, "error": "timeout"}
        except requests.exceptions.ConnectionError as e:
            logger.error(f"Ollama vision connection error: {str(e)}")
            return "", {"model": self.vision_model_name, "error": "connection_error"}
        except requests.exceptions.RequestException as e:
            logger.error(f"Ollama vision request error: {str(e)}")
            return "", {"model": self.vision_model_name, "error": str(e)}
        except Exception as e:
            logger.error(f"Ollama vision unexpected error: {str(e)}", exc_info=True)
            return "", {"model": self.vision_model_name, "error": str(e)}
